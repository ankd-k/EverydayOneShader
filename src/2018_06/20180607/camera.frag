...
void main(){
    ...
    // view matrix
    vec3 v_eye = vec3(0., 0., 0.5+0.5*sin(time*0.1));
    v_eye = vec3(cos(t*0.234), 0., sin(t*0.567))*5.;
    // v_eye = vec3(0.);
    vec3 v_target = vec3(0.);
    v_target = vec3(sin(t*0.531), sin(t*0.421), sin(t*1.684));
    v_target = vec3(cos(t), sin(t*0.2)*0.5, sin(t));
