...
vec3 snoiseVec3( vec3 x ){
  ...
  float s1 = snoise(vec3( x.y - 19.1*sin(mod(time,60.)*0.02) , x.z + 33.4 , x.x + 47.2 ));
  float s2 = snoise(vec3( x.z + 74.2*mouse.y , x.x - 124.5 , x.y + 99.4*cos(mod(time,60.)*0.03) ));
  vec3 c = vec3( s , s1 , s2 );
  return c;
...
void main(){
    ...
    float speed = 0.1*mouse.x;

    // if(1.2<position.y){
    //   velocity = vec4(0.);
    // }

    vec3 newVelocity = speed * normalize(velocity.xyz// current velocity
                                         + curlNoise(position.xyz)// curlNoise
                                         + vec3(0.,.8,0.)// up velocity
                                        );
