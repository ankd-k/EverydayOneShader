...
float sphere(in float r, in float theta, in float phi, out vec3 pos, out vec3 norm){

  float id = vertexId/vertexCount;
  // id = id*2.-1.;
  float x = cos(phi*id-0.5*PI)*sin(theta*id);
  float y = sin(phi*id-0.5*PI);
  float z = cos(phi*id-0.5*PI)*cos(theta*id);
  pos = r*vec3(x,y,z);
...
void main(){
    ...
    gl_PointSize = 3.;
    v_color = vec4(hsv2rgb(vec3(id, 1., 1.)), 1.);
    v_color = vec4(1.);
}
