...
void main(){
  ...
  og = og*2.-1.;
  float pct = ease_inout(ft, 8.);
  vec3 offset = (1.-pct)*os + pct*og;

  offset = vec3(cos(panelId + t*0.1)-0.8, 0., sin(panelId + t*0.1));
  // offset = og;
  // offset = vec3(
  //     0.5*sin(panelId*2.4+t),
  //     0.4*sin(panelId*1.8+t),
  //     0.8*sin(panelId*3.3+t)
  //   );
  offset *= 4.;

  vec3 pos = vec3(0.);
  plane(vId, offset, vec2(0.15), pos, v_uv);

  gl_Position = getCameraMatrix() * vec4(pos, 1.);
  v_position = pos;
  gl_PointSize = 1.;
  v_color = vec4(clamp(8./length(pos), 0., 1.));
}
