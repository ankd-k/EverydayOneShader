...
vec2 map(vec3 p){
  ...
  vec2 res = vec2(dfBox(pb+vec3(.0, 0., 0.), vec3(0.9*ease_inout(fract(time*0.5), 6.))), 1.);
  res = opU(res, vec2(dfTorus88(pt+vec3(0.1), vec2(2., 0.08)), 2.));
  return res;
}
...
void main(){
  ...
  vec3 tar = vec3(
      0.,//random(floor(time*0.5)),
      0.,//random(floor(time*0.5)+10000.),
      1.
    );
  // tar = tar*2.-1.;
  vec3 target = eye - tar;
