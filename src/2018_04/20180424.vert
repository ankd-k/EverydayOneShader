...
void main(){
  ...
  mat4 mInvMat = inverse(mMat);
  // view matrix
  vec3 v_eye = vec3(0., -1., 2.);
  // v_eye = vec3(2.*sin(t*.5),-1.,2.*cos(t*.5));
  ...
  // light iniit settings
  // vec3 lightIntensity = vec3(1.);
  // vec3 lightPos = vec3(2.);
  // // other init settings
  // vec3 normal = normalize(transpose(mInvMat)*vec4(v_normal, 1.)).xyz;
  // vec3 surfacePos = (mMat * vec4(pos, 1.)).xyz;
  // vec3 surfaceToLight = normalize(lightPos - surfacePos);
  // vec3 surfaceToCamera = normalize(v_eye - surfacePos);
  vec3 surfaceColor = vec3(1.);
  v_color = vec4(1., 1., 1., 1.);
  // directionalLight
  // vec3 directionalColor = vec3(1.);
  // vec3 directionalDir = vec3(0.,-1.,1.);
  // vec3 directionalNor = normalize(mInvMat * vec4(directionalDir, 0.)).xyz;
  // float diff = clamp(dot(v_normal, directionalNor), 0.1, 1.);
  // v_color *= vec4(vec3(surfaceColor * diff), 1.);
  // point Light
  vec3 pointPos = vec3(cos(t), -.5, sin(t));
  pointPos.xz *= 1.;
  lightPos = pointPos;
  vec3 pointVec = pointPos - (mMat*vec4(pos, 0.)).xyz;
  vec3  invLight  = normalize(mInvMat * vec4(pointVec, 0.0)).xyz;
  vec3  invEye    = normalize(mInvMat * vec4(v_eye, 0.0)).xyz;
  ...
  float attenuation = 1./ (1. + 0.05*pow(length(pointPos-(mMat*vec4(pos, 0.)).xyz), 2.));
  v_color *= vec4(vec3(diffuse), 1.0) + vec4(vec3(specular), 1.0) + ambient;
  v_color *= attenuation;
