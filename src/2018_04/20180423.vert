...
void main(){
  ...
  // view matrix
  vec3 v_eye = vec3(2.*cos(t), -1., 2.*sin(t));
  v_eye = vec3(0.,-1.,2.);
  ...
  vec3 invLight = normalize(mInvMat * vec4(lightDir, 0.)).xyz;
  float diffuse = clamp(dot(v_normal, invLight), 0.1, 1.);
